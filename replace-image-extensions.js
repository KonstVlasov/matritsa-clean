...
/**
 * Скрипт для замены ссылок на изображения в исходных файлах
 * Заменяет .png на .webp и .jpg/.jpeg на .webp
 */

// Путь к директории с исходными файлами
const SRC_DIR = 'u2865078/data/www/app/frontend/src';
...
const IMAGE_PATTERNS = [
  // Для путей в кавычках (например, в импортах, src атрибутах и т.д.)
  {
  ...
  // Для импортов изображений
  {
    regex: /(import\s+[^'"]*\s+from\s+['"])([^'"]*\.(?:png|jpg|jpeg))(['"])/gi,
    replacer: (match, prefix, path, quote) => {
      // Проверяем, не является ли это уже webp
      if (path.toLowerCase().endsWith('.webp')) {
        return match;
      }
  ...
  // Для require() вызовов
  {
    regex: /(require\s*\(\s*['"])([^'"]*\.(?:png|jpg|jpeg))(['"])/gi,
    replacer: (match, prefix, path, quote) => {
      // Проверяем, не является ли это уже webp
      if (path.toLowerCase().endsWith('.webp')) {
        return match;
...
async function main() {
  try {
    console.log(`🔍 Поиск файлов в директории: ${SRC_DIR}`);
    ...
    const backupDir = `./${path.basename(SRC_DIR)}_backup_${Date.now()}`;
